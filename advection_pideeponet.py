"""DeepXDE 官方 PI-DeepONet 示例：一维平流方程算子。

代码来源：
https://raw.githubusercontent.com/lululxvi/deepxde/master/examples/operator/advection_aligned_pideeponet.py
https://raw.githubusercontent.com/lululxvi/deepxde/master/examples/operator/advection_unaligned_pideeponet.py
...
def initial_value(_x, v):
    """初值条件 u(x, 0)=v(x)，其中 v 是待学习算子的输入函数。"""
...
def build_operator_data(config, data_pde):
    ...
    if config["mode"] == "aligned":
        # function_variables=[0] 表示输入函数 v 只依赖空间变量 x，不依赖时间 t。
...
def analytical_solution(x, t):
    """初值 v(x)=sin(2*pi*x) 时，平流方程解为 u(x,t)=v(x-t)。"""
...
def save_prediction_figure(x, t, eval_points, branch_values, u_pred, u_true, output_dir, mode):
    """保存初值函数、真实解、预测解和误差图。"""
